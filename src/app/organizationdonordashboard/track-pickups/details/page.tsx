      ...
      fetch("/api/organization-donations")
        ...
        .then((data) => {
          const found = donationId
            ? data.find((entry: Donation) => String(entry.id) === String(donationId))
            : null;
    ...
    <div className="min-h-screen bg-gradient-to-b from-blue-200 to-blue-100 flex flex-col items-center py-8 px-2">
      <div className="w-full max-w-2xl bg-white/90 rounded-3xl shadow-2xl border border-sky-100 p-6 sm:p-10 animate-fadeinup relative">
        {details.status === "cancelled" ? (
          ...
          <>
            <h1 className="text-2xl font-bold text-sky-700 mb-2">Your pickup is confirmed</h1>
            <p className="text-sky-700 mb-4">Status: <span className={details.status === "accepted" ? "text-green-600" : "text-yellow-600"}>{details.status}</span></p>
            <div className="mb-6">
              <div className="flex items-center gap-2 mb-2">
                {(details.timeline || []).map((step, index) => (
                  <React.Fragment key={step.step}>
                    ...
                    {index < (details.timeline?.length || 0) - 1 && <div className="w-6 h-0.5 bg-gray-300" />}
                  </React.Fragment>
                ))}
              </div>
              <div className="flex gap-2 mt-2 flex-wrap">
                <span className="inline-block bg-sky-100 text-sky-700 px-3 py-1 rounded-full text-xs font-semibold">Status: {details.status}</span>
                <span className="inline-block bg-green-100 text-green-700 px-3 py-1 rounded-full text-xs font-semibold">Pickup window: Today, 6:30-7:30 PM</span>
            ...
            <div className="flex items-center gap-4 mb-6">
              <Image
                src={details.volunteer?.photoUrl || "/images/abdulpic.jpg"}
                alt="Volunteer"
                width={64}
                height={64}
              ...
              <div className="flex-1">
                <div className="font-semibold text-sky-800 text-lg flex items-center gap-2">
                  {details.volunteer?.name || "Volunteer"}
                  {details.volunteer?.ngoVerified && <span className="ml-1 text-xs bg-green-200 text-green-800 px-2 py-0.5 rounded-full">Verified</span>}
                </div>
                <div className="text-gray-600 text-sm">ID: {details.volunteer?.id || "N/A"} • {details.volunteer?.ngo || "N/A"}</div>
                <div className="text-gray-600 text-sm">Vehicle: {details.volunteer?.vehicle || "N/A"}</div>
                <div className="flex gap-2 mt-2">
                  <a href={`tel:${details.volunteer?.phone || "#"}`} className="bg-sky-400 hover:bg-sky-500 text-white px-3 py-1 rounded-lg text-xs font-semibold">Call</a>
                  <a href={`sms:${details.volunteer?.phone || "#"}`} className="bg-sky-200 hover:bg-sky-300 text-sky-800 px-3 py-1 rounded-lg text-xs font-semibold">Message</a>
                </div>
              </div>
            </div>
          </>
        ...
        <div className="mb-6 text-sm text-yellow-700 bg-yellow-50 border-l-4 border-yellow-400 p-3 rounded">
          Volunteer will show NGO ID at pickup. Verify the ID and match code: <span className="font-bold">{details.volunteer?.verificationId || "N/A"}</span>.
        </div>
        ...
        <div className="mb-6">
          ...
          <ul className="mb-2">
            {(details.items || []).map((item, index) => (
              <li key={index} className="text-gray-700 text-sm">{item.name} × {item.quantity} <span className="text-gray-500">({item.details})</span></li>
            ))}
          </ul>
          <div className="text-gray-700 text-sm">Pickup address: {details.address}</div>
          <div className="text-gray-700 text-sm">Special instructions: {details.instructions}</div>
          <div className="text-gray-700 text-sm">Donation ID: {details.id}</div>
        </div>
        ...
        <div className="mb-6 text-sm bg-blue-50 border-l-4 border-sky-300 p-3 rounded">
          <div className="text-gray-900">Volunteer code: <span className="font-bold">{details.volunteer?.verificationId || "N/A"}</span></div>
          <div className="text-gray-900">Your OTP: <span className="font-bold">{details.donorOtp}</span></div>
        ...
        <div className="mb-2">
          ...
          <a href={details.receipts?.pickupSlipUrl || "#"} className="text-sky-600 underline mr-4">Download pickup slip</a>
          <a href={details.receipts?.certificateUrl || "#"} className="text-sky-600 underline">Download certificate</a>
        </div>
        ...
        {showEditNotes && (
          <div className="fixed inset-0 bg-black/30 flex items-center justify-center z-50">
            <div className="bg-white rounded-xl p-6 shadow-xl w-80">
              ...
              <textarea className="w-full border rounded p-2 mb-4" rows={3} defaultValue={details.instructions}></textarea>
