...
export default function ManageDonationsPage() {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center bg-gradient-to-b from-[#b3e0ff] to-[#e0f7fa] py-8 px-2">
      <div className="w-full max-w-2xl bg-white/90 rounded-3xl shadow-2xl border border-sky-100 flex flex-col items-center py-8 px-4 sm:px-10 animate-fadeinup">
        ...
        <h2 className="text-2xl font-bold text-sky-700 mb-8 text-center">Add Public Donation</h2>
        {/* 2-column grid layout for all items */}
        <div className="w-full grid grid-cols-1 sm:grid-cols-2 gap-6 mb-8">
          {donationItems.map((item) => (
            <Link
              key={item.key}
              href={`/publicdonordashboard/manage-donations/donate?item=${item.key}`}
              className="w-full"
            >
  ...
  <h3 className="text-xl font-bold text-sky-700 mb-2 mt-2 w-full text-center">Your Public Donations</h3>
